...
// ── Load assets at cold start ──────────────────────────────────────────
const FONTS_DIR = path.join(process.cwd(), "fonts");
const ASSETS_DIR = path.join(process.cwd(), "assets");

let poppinsSemiBoldBytes, poppinsMediumBytes, geistRegularBytes;
let logoPng, emailIconPng, phoneIconPng, webIconPng, pinIconPng;

function loadAssets() {
  if (poppinsSemiBoldBytes) return; // already loaded
  poppinsSemiBoldBytes = fs.readFileSync(path.join(FONTS_DIR, "Poppins-SemiBold.ttf"));
  poppinsMediumBytes   = fs.readFileSync(path.join(FONTS_DIR, "Poppins-Medium.ttf"));
  geistRegularBytes    = fs.readFileSync(path.join(FONTS_DIR, "Geist-Regular.ttf"));
  logoPng              = fs.readFileSync(path.join(ASSETS_DIR, "logo.png"));
  emailIconPng         = fs.readFileSync(path.join(ASSETS_DIR, "email-icon.png"));
  phoneIconPng         = fs.readFileSync(path.join(ASSETS_DIR, "phone-icon.png"));
  webIconPng           = fs.readFileSync(path.join(ASSETS_DIR, "web-icon.png"));
  pinIconPng           = fs.readFileSync(path.join(ASSETS_DIR, "pin-icon.png"));
}
